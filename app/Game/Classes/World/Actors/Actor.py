...
class Actor(ABC):
  ...
  def interaction(self, other=None, context=""):
    ...
    if (actions_propose):
      action_key = self.ask("Que voulez-vous faire ?", list(actions_propose.keys()), True)
      action = actions_propose[action_key]
      [action_object, action_function] = action
      action_function(action_object, other)
